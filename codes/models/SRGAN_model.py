...
import numpy as np
import torchvision
from .loss import cal_dft, cal_dct
    ...
    def optimize_parameters(self, step):
        ...
        if step % self.D_update_ratio == 0 and step > self.D_init_iters:
            if self.cri_pix:  # pixel loss
                ...
                # DCT loss
                DCT_loss = cal_dct(self.fake_H, self.var_H, 20) * 0.01
                l_g_total = l_g_total + DCT_loss

                if self.BTMap_on == 1:
                    l_g_pix_BTMap = self.cri_pix(self.Condi, torch.unsqueeze(self.var_LPIPS_maps_best_idx[:, 0, :, :],
                                                                             dim=1)) * 0.01
        ...
        self.log_dict['l_g_DCT'] = DCT_loss.item()
